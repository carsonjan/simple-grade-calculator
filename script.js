...
function addItem() {
    ...
    newItemGrade.setAttribute('class', 'itemGrade');
    newItemGrade.setAttribute("style", "margin-left: 10px;");
    newItem.appendChild(newItemGrade);

    const newFindMinButton = document.createElement("button");
    newFindMinButton.textContent = "find min grade to pass";
    newFindMinButton.setAttribute("class", "findMinGrade");
    newFindMinButton.setAttribute("style", "margin-left: 10px;");
    newFindMinButton.addEventListener('click', () => findMinimumGradeNeeded(newItemGrade));
...
function findMinimumGradeNeeded(inputBox) {
    ...
    while (percentStringToFloat(inputBox.value) < 100) {
        ...
        if (percentStringToFloat(totalGradeValue.textContent) >= percentStringToFloat(minPassingGradeValue)) return;
        inputBox.value = (percentStringToFloat(inputBox.value) + 0.5).toString();
...
function updateTotalGrade() {
    ...
    items.forEach(item => {
        ...
        let weight = parseFloat(item.querySelector('.itemWeight').textContent.slice(1, -2)) / 100.0;
...
document.addEventListener("keydown", (event) => {if (event.key == "Enter") addItem()});
